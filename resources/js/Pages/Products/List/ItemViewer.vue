<template>
  ...
  >
    <div class="grid grid-cols-12 gap-6">
      ...
      <!-- Product Info -->
      <div class="lg:col-span-7 col-span-12 flex flex-col gap-4">
        <!-- Status & Brand -->
        <div class="flex items-center gap-3 flex-wrap">
          ...
          >
            <i class="fa fa-tag mr-1" />{{ product.brand.name }}
          </span>
        </div>
      ...
      <template v-if="product?.description">
        <div class="col-span-12">
          ...
          <p class="text-color m-0 leading-relaxed">
            {{ product.description }}
          </p>
        </div>
      </template>
        ...
        <div class="col-span-12">
          ...
          <DataTable :value="product.variants" size="small" show-gridlines>
            ...
            <Column field="status" :header="t('Status')" style="width: 100px">
              <template #body="{ data: variant }">
                <Tag
                  v-if="variant.status === 'active'"
                  severity="success"
                  :value="t('Active')"
                  class="text-xs"
                />
                <Tag
                  v-else-if="variant.status === 'inactive'"
                  severity="warn"
                  :value="t('Inactive')"
                  class="text-xs"
                />
