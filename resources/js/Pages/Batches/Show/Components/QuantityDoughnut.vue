    ...
    tooltip: {
      callbacks: {
        label: (context: { raw: number }) => {
          ...
          const percent = props.initial ? Math.round((value / props.initial) * 100) : 0;
...
<template>
  <div class="relative flex items-center justify-center" style="height: 160px">
    ...
    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
      <span class="text-2xl font-bold">{{ initial ? Math.round(((sold + remaining + missing) / initial) * 100) : 0 }}%</span>
