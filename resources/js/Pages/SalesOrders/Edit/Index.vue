...
const lineItems = ref<LineItem[]>(
  (props.order.items ?? []).map((item) => ({
    ...
    variant_identity: item.product_variant?.option_values ?? null,
    variant_label: item.product_variant?.option_values ?? item.sale_unit?.name ?? item.product_variant?.identifier ?? t("Unit"),
