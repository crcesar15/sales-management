...
<template>
  <Dialog :visible="visible" modal :header="t('Product Handover')" class="w-full max-w-4xl" @update:visible="emit('update:visible', $event)">
    ...
    <DataTable :value="allocations" data-key="id" striped-rows>
      ...
      <Column :header="t('Expiry')"><template #body="{ data }">{{ data.expiry ? formatDate(data.expiry) : "---" }}</template></Column>
    </DataTable>
    <template #footer>
      <Button :label="t('Cancel')" severity="secondary" text @click="emit('update:visible', false)" />
      <Button :label="t('Confirm Handover')" icon="fa fa-box" :loading="processing" @click="emit('confirm')" />
    </template>
  </Dialog>
</template>
