<script setup lang="ts">
import {
  DataTable,
  Card,
  Column,
  Toast,
  Button,
  InputText,
  IconField,
  InputIcon,
  ConfirmDialog,
  SelectButton,
  Tag,
  useToast,
  useConfirm,
  type DataTablePageEvent,
  type DataTableSortEvent,
} from "primevue";

import AppLayout from "@layouts/admin.vue";
import { useDatetimeFormatter } from "@composables/useDatetimeFormatter";

import { computed, ref, watch } from "vue";
import { router, useForm } from "@inertiajs/vue3";
import { route } from "ziggy-js";
import type { CustomerResponse } from "@/Types/customer-types";
import { useI18n } from "vue-i18n";

defineOptions({ layout: AppLayout });

const props = defineProps<{
  customers: {
    data: CustomerResponse[];
    meta: {
      current_page: number;
      last_page: number;
      per_page: number;
      total: number;
    };
  };
  filters: {
    filter?: string | null;
    status?: string;
    order_by?: string;
    order_direction?: string;
    per_page?: number;
  };
}>();

const toast = useToast();
const confirm = useConfirm();
const { t } = useI18n();
const { formatDatetime } = useDatetimeFormatter();

const filter = ref(props.filters.filter ?? "");
const status = ref(props.filters.status ?? "active");
const sortField = ref(props.filters.order_by ?? "first_name");
const sortOrder = ref(props.filters.order_direction === "desc" ? -1 : 1);

const customers = computed(() =>
  props.customers.data.map((item) => ({
    ...item,
    created_at: formatDatetime(item.created_at),
    updated_at: formatDatetime(item.updated_at),
  })),
);

let filterTimer: ReturnType<typeof setTimeout>;
watch(filter, (val) => {
  clearTimeout(filterTimer);
  filterTimer = setTimeout(() => {
    router.visit(route("customers"), {
      data: {
        filter: val,
        status: status.value,
        order_by: sortField.value,
        order_direction: sortOrder.value === -1 ? "desc" : "asc",
      },
      preserveState: true,
      replace: true,
    });
  }, 300);
});

watch(status, (val) => {
  router.visit(route("customers"), {
    data: {
      status: val,
      filter: filter.value,
      order_by: sortField.value,
      order_direction: sortOrder.value === -1 ? "desc" : "asc",
    },
    preserveState: true,
    replace: true,
  });
});

const onPage = (event: DataTablePageEvent) => {
  router.visit(route("customers"), {
    data: {
      page: event.page + 1,
      per_page: event.rows,
      order_by: sortField.value,
      order_direction: sortOrder.value === -1 ? "desc" : "asc",
      filter: filter.value,
      status: status.value,
    },
    preserveState: true,
    replace: true,
  });
};

const onSort = (event: DataTableSortEvent) => {
  sortField.value = typeof event.sortField === "string" ? event.sortField : "first_name";
  sortOrder.value = event.sortOrder ?? 1;
  router.visit(route("customers"), {
    data: {
      order_by: sortField.value,
      order_direction: sortOrder.value === -1 ? "desc" : "asc",
      filter: filter.value,
      status: status.value,
    },
    preserveState: true,
    replace: true,
  });
};

const editCustomer = (customer: CustomerResponse) => {
  router.visit(route("customers.edit", customer.id));
};

const deleteCustomer = (id: number) => {
  confirm.require({
    message: t("Are you sure you want to delete this customer?"),
    header: t("Confirm"),
    icon: "fas fa-exclamation-triangle",
    rejectLabel: t("Cancel"),
    acceptLabel: t("Delete"),
    rejectClass: "p-button-secondary",
    accept: () => {
      const form = useForm({});
      form.delete(route("customers.destroy", id), {
        onSuccess: () => {
          toast.add({
            severity: "success",
            summary: t("Success"),
            detail: t("Customer deleted successfully"),
            life: 3000,
          });
        },
        onError: () => {
          toast.add({
            severity: "error",
            summary: t("Error"),
            detail: t("Could not delete customer"),
            life: 3000,
          });
        },
      });
    },
  });
};
</script>

<template>
  <div>
    <div class="flex flex-row justify-between mb-3">
      <h2 class="text-2xl font-bold flex items-end m-0">
        {{ t("Customers") }}
      </h2>
      <Button
        v-can="'customer.create'"
        :label="t('Add Customer')"
        icon="fa fa-add"
        raised
        class="ml-2 uppercase"
        @click="$inertia.visit(route('customers.create'))"
      />
    </div>
    <ConfirmDialog />
    <Toast />
    <Card>
      <template #content>
        <DataTable
          :value="customers"
          resizable-columns
          lazy
          :total-records="props.customers.meta.total"
          :rows="props.customers.meta.per_page"
          :first="(props.customers.meta.current_page - 1) * props.customers.meta.per_page"
          paginator
          sort-field="first_name"
          :sort-order="1"
          @page="onPage($event)"
          @sort="onSort($event)"
        >
          <template #empty>
            <div class="flex flex-col items-center py-8 text-surface-400">
              <i class="fa fa-folder-open text-4xl mb-3"></i>
              <span>{{ t("No customers found") }}</span>
            </div>
          </template>
          <template #header>
            <div class="grid grid-cols-12">
              <div class="md:col-span-6 col-span-12 flex md:justify-start justify-center">
                <SelectButton
                  v-model="status"
                  :allow-empty="false"
                  :options="[
                    { label: t('All'), value: 'all' },
                    { label: t('Active'), value: 'active' },
                    { label: t('Inactive'), value: 'inactive' },
                  ]"
                  option-label="label"
                  option-value="value"
                />
              </div>
              <div
                class="flex xl:col-span-3 xl:col-start-10 lg:col-span-4 lg:col-start-9 md:col-span-6 md:col-start-7 col-span-12 md:justify-end justify-center"
              >
                <IconField icon-position="left" class="w-full">
                  <InputIcon class="fa fa-search" />
                  <InputText v-model="filter" :placeholder="t('Search')" fluid />
                </IconField>
              </div>
            </div>
          </template>
          <Column field="first_name" :header="t('First Name')" sortable>
            <template #body="{ data }">
              <span :class="{ 'opacity-50': data.status === 'inactive' }">
                {{ data.first_name || "—" }}
              </span>
            </template>
          </Column>
          <Column field="last_name" :header="t('Last Name')" sortable>
            <template #body="{ data }">
              <span :class="{ 'opacity-50': data.status === 'inactive' }">
                {{ data.last_name || "—" }}
              </span>
            </template>
          </Column>
          <Column field="email" :header="t('Email')" sortable>
            <template #body="{ data }">
              <span :class="{ 'opacity-50': data.status === 'inactive' }">
                {{ data.email || "—" }}
              </span>
            </template>
          </Column>
          <Column field="phone" :header="t('Phone')" sortable>
            <template #body="{ data }">
              <span :class="{ 'opacity-50': data.status === 'inactive' }">
                {{ data.phone || "—" }}
              </span>
            </template>
          </Column>
          <Column field="tax_id" :header="t('Tax ID')" sortable>
            <template #body="{ data }">
              <span :class="{ 'opacity-50': data.status === 'inactive' }">
                {{ data.tax_id || "—" }}
              </span>
            </template>
          </Column>
          <Column field="status" :header="t('Status')" sortable>
            <template #body="{ data }">
              <Tag v-if="data.status === 'active'" severity="success" :value="t('Active')" rounded />
              <Tag v-else severity="warn" :value="t('Inactive')" rounded />
            </template>
          </Column>
          <Column field="sales_orders_count" :header="t('Sales Orders')" :pt="{ columnHeaderContent: 'justify-center' }">
            <template #body="{ data }">
              <div class="flex justify-center">
                <span class="inline-flex items-center justify-center rounded-full bg-surface-100 dark:bg-surface-700 px-3 py-1 text-sm">
                  {{ data.sales_orders_count ?? 0 }}
                </span>
              </div>
            </template>
          </Column>
          <Column field="actions" :header="t('Actions')" :pt="{ columnHeaderContent: 'justify-center' }">
            <template #body="{ data }">
              <div class="flex justify-center gap-2">
                <Button
                  v-can="'customer.edit'"
                  v-tooltip.top="t('Edit')"
                  icon="fa fa-edit"
                  text
                  size="large"
                  rounded
                  @click="editCustomer(data)"
                />
                <Button
                  v-can="'customer.delete'"
                  v-tooltip.top="t('Delete')"
                  icon="fa fa-trash"
                  text
                  size="large"
                  rounded
                  @click="deleteCustomer(data.id)"
                />
              </div>
            </template>
          </Column>
        </DataTable>
      </template>
    </Card>
  </div>
</template>